...
const checkToUniqURL = (state) => {
  ...
  const { feeds } = state;
  const repeatedLinks = feeds ? feeds.filter((feed) => feed.link === link) : [];
...
const validateForm = (state) => {
  ...
  const schema = yup.object().shape({
    url: yup.string().url(),
    unique: yup.array().length(0),
    response: yup.string().matches(/<\/rss>/),
  });
  ...
  try {
    ...
    const errors = _.keyBy(error.inner, 'path');
    return errors;
  }
};
...
const render = (feeds) => {
  ...
  document.querySelector('.feeds-container .feeds').append(feedsTitle);
  document.querySelector('.feeds-container .feeds').append(feedsList);
  document.querySelector('.feeds-container .posts').append(postsTitle);
  document.querySelector('.feeds-container .posts').append(postsList);
...
const updatePosts = (state) => {
  state.feeds.forEach((feed) => {
    axios.get(`https://hexlet-allorigins.herokuapp.com/get?url=${encodeURIComponent(feed.link)}`)
      .then((response) => {
        const newFeed = makeNewFeed(response.data.contents, feed.link);
        console.log(state.feeds, newFeed);
      });
  });
...
export {
  validateForm, render, makeNewFeed, renderErrors, updatePosts,
};
